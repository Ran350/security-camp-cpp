...
class TextFileReader::Impl
{
  public:
    ...
    void close() { m_binaryFileReader.close(); }
...
TextFileReader::TextFileReader() : m_pImpl{std::make_shared<Impl>()} {}
...
bool TextFileReader::isOpen() const noexcept { return m_pImpl->isOpen(); }

TextFileReader::operator bool() const noexcept { return m_pImpl->isOpen(); }
...
void TextFileReader::close() { m_pImpl->close(); }
