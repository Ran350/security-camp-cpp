...
class Image
{
  public:
    ...
    Image(int32 width, int32 height, const Color &color = Palette::White)
        : m_pixels{(static_cast<size_t>(width) * static_cast<size_t>(height)),
                   color},
          m_size{width, height}
    {
    }
    ...
    /// @param color 画像の初期色
    [[nodiscard]]
    Image(std::integral auto width, std::integral auto height,
    ...
    [[nodiscard]]
    friend constexpr bool operator==(const Image &lhs,
                                     const Image &rhs) noexcept = default;
    ...
    /// @return 指定した行の先頭ポインタ
    Color *operator[](size_t y) { return (m_pixels.data() + (y * m_size.x)); }
