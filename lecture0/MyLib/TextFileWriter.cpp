...
class TextFileWriter::Impl
{
  public:
    ...
    void close() { m_binaryFileWriter.close(); }
...
TextFileWriter::TextFileWriter() : m_pImpl{std::make_shared<Impl>()} {}
...
TextFileWriter::operator bool() const noexcept { return m_pImpl->isOpen(); }

void TextFileWriter::close() { m_pImpl->close(); }

void TextFileWriter::write(const std::string_view s) { m_pImpl->write(s); }
