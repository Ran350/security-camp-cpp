...
struct BMPHeader
{
    ...
    /// @return BMP ヘッダ
    [[nodiscard]]
    static constexpr BMPHeader Make(int32 width, int32 height,
                                    uint32 size_bytes) noexcept
    {
...
bool SaveBMP(const Image &image, const std::string_view path)
{
    ...
    const uint32 strideBytes =
        (width * 3 +
         width % 4); // BMP の 1 行は 4 の倍数バイトにアラインメントされる
...
Image LoadBMP(const std::string_view path)
{
    ...
    // 下から上方向に格納？
    const bool reverse =
        (header.biHeight > 0); // BMP は下から上に書き込まれる。biHeight
                               // が負の場合は上から下に書き込まれている
